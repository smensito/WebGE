...
import static engine.data.Common.currentDateTimeAsFormattedString;
import engine.data.NormalizedDataTable;
...
import engine.util.MyCompiler;
import engine.util.MyLoader;
import engine.util.UtilStats;
...
public class GramEvalTemporalModel extends AbstractProblemGE {
    ...
    public GramEvalTemporalModel(Properties properties, int threadId, int numObjectives) throws IOException {
        super(properties.getProperty(engine.data.Common.BNF_PATH_FILE_PROP), numObjectives,
              Integer.valueOf(properties.getProperty(engine.data.Common.CHROMOSOME_LENGTH_PROP)),
              Integer.valueOf(properties.getProperty(engine.data.Common.MAX_WRAPS_PROP)),
              AbstractProblemGE.CODON_UPPER_BOUND_DEFAULT);
        this.properties = properties;
        this.threadId = threadId;
        compiler = new MyCompiler(properties,logger);
        boolean normalizeData = false;
        if (properties.getProperty(engine.data.Common.NORMALIZED_DATA_PROP) != null) {
            normalizeData = Boolean.valueOf(properties.getProperty(engine.data.Common.NORMALIZED_DATA_PROP));
        } 
        
        if (this.properties.getProperty(engine.data.Common.REAL_DATA_COPIED_PROP) != null) {
            realDataCopied = Integer.valueOf(this.properties.getProperty(engine.data.Common.REAL_DATA_COPIED_PROP));
        } else {
            realDataCopied = 0;
        }
        ...
        dataTable = new NormalizedDataTable(this, properties.getProperty(engine.data.Common.TRAINING_PATH_PROP),
                properties.getProperty(engine.data.Common.VALIDATION_PATH_PROP),
                Double.valueOf(properties.getProperty(engine.data.Common.ERROR_THRESHOLD_PROP)),
                Boolean.valueOf(properties.getProperty(engine.data.Common.VIEW_RESULTS_PROP)),normalizeData,
                Integer.valueOf(properties.getProperty(engine.data.Common.MODEL_WIDTH_PROP))+realDataCopied);
    }

    public boolean generateAndCompileEvaluationFile(ArrayList<String> exprList) {
        StringBuilder currentJavaFile = new StringBuilder();

        currentJavaFile.append("public class PopEvaluator").append(threadId).append(" extends engine.algorithm.AbstractPopEvaluator {\n\n");
    ...
    public void evaluate(Solutions<Variable<Integer>> solutions) {
        ...
        if (generateAndCompileEvaluationFile(solStrings)) {
            ...
            try {
                ...
                if (this.properties.getProperty(engine.data.Common.REAL_DATA_COPIED_PROP) != null) {
                    evaluator.setRealDataCopied(Integer.valueOf(this.properties.getProperty(engine.data.Common.REAL_DATA_COPIED_PROP)));
                }
                if (this.properties.getProperty(engine.data.Common.MODEL_WIDTH_PROP) != null) {
                    evaluator.setModelWidth(Integer.valueOf(this.properties.getProperty(engine.data.Common.MODEL_WIDTH_PROP)));
                }                    
            } catch (Exception ex) {
            ...
            for (int i = 0; i < solutions.size(); ++i) {
                ...
                if (super.numberOfObjectives == 2) {
                    ...
                    NormalizedDataTable.OBJECTIVE = Integer.valueOf(this.properties.getProperty(engine.data.Common.OBJECTIVES_PROP));
...
//                        logger.info("I have a NaN number here: "+problem.generatePhenotype(solutions.get(0)).toString());
    ...
    public static Properties loadProperties(String propertiesFilePath) {
        ...
        try {
            ...
            File clsDir = new File(properties.getProperty(engine.data.Common.WORK_DIR_PROP));
    ...
    public static void runGE(Properties properties, int threadId, Observer obs) {
        ...
        // Log population:
        logPopulation = Integer.valueOf(properties.getProperty(engine.data.Common.LOG_POPULATION_PROP));
        if (logPopulation > 0) {
            gen = 0;
            String fileName = "Log_Population_" + currentDateTimeAsFormattedString() + ".csv";
            logPopulationOutputFile = properties.getProperty(engine.data.Common.LOGGER_BASE_PATH_PROP) + File.separator + fileName;
        ...
        int numObjectives = 1;
        if ((properties.getProperty(engine.data.Common.OBJECTIVES_PROP) != null)
                && (Integer.valueOf(properties.getProperty(engine.data.Common.OBJECTIVES_PROP)) == 2)) {
            numObjectives = 2;
        }
        problem = null;
        try {
        ...
        if (properties.getProperty(engine.data.Common.CROSSOVER_PROB_PROP) != null) {
            crossOverProb = Double.valueOf(properties.getProperty(engine.data.Common.CROSSOVER_PROB_PROP));
        }
        double mutationProb = 1.0 / problem.reader.getRules().size();
        if (properties.getProperty(engine.data.Common.MUTATION_PROB_PROP) != null) {
            mutationProb = Double.valueOf(properties.getProperty(engine.data.Common.MUTATION_PROB_PROP));
        }
        
        int tournamentSize = 2;
        if (properties.getProperty(engine.data.Common.TOURNAMENT_SIZE_PROP) != null) {
            tournamentSize = Integer.valueOf(properties.getProperty(engine.data.Common.TOURNAMENT_SIZE_PROP));
        }
        
        // Set weight for CEG penalty
        UtilStats.setCEGPenalties(properties);
        
        // Second create the engine.algorithm
        ...
        if (numObjectives == 2) {
            algorithm = new ModifiedNSGAII(problem, Integer.valueOf(properties.getProperty(engine.data.Common.NUM_INDIVIDUALS_PROP)), Integer.valueOf(properties.getProperty(engine.data.Common.NUM_GENERATIONS_PROP)), mutationOperator, crossoverOperator, selectionOp);
        } else {
            algorithm = new SimpleGeneticAlgorithm<>(problem, Integer.valueOf(properties.getProperty(engine.data.Common.NUM_INDIVIDUALS_PROP)), Integer.valueOf(properties.getProperty(engine.data.Common.NUM_GENERATIONS_PROP)), true, mutationOperator, crossoverOperator, selectionOp);
        }
        if (obs != null) {
        ...
        int numExecutions = 1;
        if (properties.getProperty(engine.data.Common.NUM_EXECUTIONS) != null) {
            numExecutions = Integer.valueOf(properties.getProperty(engine.data.Common.NUM_EXECUTIONS));
        }
     ...
     * is the reference (real) engine.data values.
     * 
     * @param strSols
     * @return 
     */
    public ArrayList<double[]> validateAndReturnEvaluations(ArrayList<String> strSols) {
        ...
        if (validateSolutions(strSols)) {
            ...
            // First model is used to include real engine.data:
    ...
    public ArrayList<String> validateAndReturnEvaluationsAsString(ArrayList<String> strSols, boolean includeInputs) {
        ...
        if (validateSolutions(strSols)) {
            ...
            // First model is used to include real engine.data and input variables:
            evaluator.evaluateExpression(0);
            ArrayList<double[]> timeTable = evaluator.getTimeTable();                
            int numCols = timeTable.get(0).length; 
